...
export default class ManageDepartment extends React.PureComponent {
  ...
  componentDidMount() {
    if((this.state.action == 'edit' || this.state.action == 'view') && this.props.info) {
      const department = this.props.info.departments.find(departmentObj => (departmentObj.id == this.props.match.params.id));
  ...
  render() {
    return (
      <div>
        ...
        {(this.props.department.id || this.state.action =='add') && this.props.info && 
          <DepartmentForm
