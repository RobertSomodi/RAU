...
export default class ManageUser extends React.PureComponent {
  ...
  async componentDidUpdate(prevProps) {
    if (
      this.props.user.storeId &&
      prevProps.user.storeId != this.props.user.storeId
    ) {
      await this.props.getDepartmentsByStoreId(this.props.user.storeId)
    }
    if (
      this.props.user.departmentId &&
      prevProps.user.departmentId != this.props.user.departmentId
    ) {
  ...
  render() {
    return (
      <div>
        ...
        {(this.props.user.id || this.state.action == 'add') &&
          this.props.info && (
            <UserForm
