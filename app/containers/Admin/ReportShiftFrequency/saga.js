...
import { getDepartmentsByStoreIdSuccess, getDepartmentsByStoreIdError, getReportShiftFrequencySuccess, getReportShiftFrequencyError } from './actions';
import { GET_DEPARTMENTS_BY_STORE_ID, GET_REPORT_SHIFT_FREQUENCY } from './constants';
...
export function* getReportShiftFrequency() {
  ...
  const endDate = `${moment(month).format('YYYY-MM')}-${moment(month).daysInMonth()}`;
  const requestURL = `http://localhost:9890/secureApi/schedule/reports/shiftreport/${searchOptions.storeId}/${searchOptions.departmentId}/${startDate}/${endDate}`;
  const requestOptions = {
    method: 'GET',
    headers: { 'Content-Type': 'application/json', Token: user.token }
  };
  ...
  try {
    ...
    yield put(getReportShiftFrequencySuccess(schedule));
  } catch (err) {
    yield put(getReportShiftFrequencyError(err));
  }
}
...
export default function* watchAll() {
  yield all([
    ...
    takeLatest(GET_REPORT_SHIFT_FREQUENCY, getReportShiftFrequency)
  ]);
}
