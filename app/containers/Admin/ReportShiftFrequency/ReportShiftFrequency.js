        ...
        if (this.props.reportShiftFrequency[key].shifts) {
          if (this.props.reportShiftFrequency[key].shifts[shift.id]) {
            row.push(
              <span>
                {`Count: ${
                  this.props.reportShiftFrequency[key].shifts[shift.id]
                }`}
                <br></br>
                {`Percent: ${
                  this.props.reportShiftFrequency[key].percent[shift.id]
                }`}
              </span>
            )
          } else {
    ...
    if (
      ...
      return (
        <div>
          ...
          />
          {this.props.reportShiftFrequency &&
            Object.keys(this.props.reportShiftFrequency).length > 0 && (
              <ReportTable
